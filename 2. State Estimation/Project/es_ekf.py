...
L_jac = np.zeros([9, 6])
L_jac[3:, :] = np.eye(6)  # motion model noise jacobian L = df/dw assuming IMU measurements are not noisy ! 
H_jac = np.zeros([3, 9])
H_jac[:, :3] = np.eye(3)  # measurement model jacobian H = df/dx
...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    R = sensor_var * np.identity(3); # x, y, z measurements
    K = p_cov_check.dot(H_jac.T).dot(np.linalg.inv(H_jac.dot(p_cov_check).dot(H_jac.T) + R));
    
    # 3.2 Compute error state
    errState = K.dot(y_k - p_check);

    # 3.3 Correct predicted state
    delta_p = errState[:3];
    delta_v = errState[3:6];
    delta_om = errState[6:]; # omega (phi, theta, epsi)
    
    p_hat = p_check + delta_p;
    v_hat = v_check + delta_v;
    q_hat = Quaternion(euler=angle_normalize(delta_om)).quat_mult_left(q_check); # q_check X delta_om
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    rotationMatrix = Quaternion(*q_est[k-1]).to_mat(); # 3x3 # q_est is 4x1 array # Motion Model in notes folder
    p_est[k] = p_est[k-1] + delta_t*v_est[k-1] + (delta_t**2/2)*(rotationMatrix.dot(imu_f.data[k-1]) + g);
    v_est[k] = v_est[k-1] + delta_t*(rotationMatrix.dot(imu_f.data[k-1]) + g);
    q_est[k] = Quaternion(axis_angle=angle_normalize(imu_w.data[k-1]*delta_t)).quat_mult_right(q_est[k-1]); 
    # idealized IMU which provide angluar velocities instead of rates.
    # axis_angle is how much angle of rotation about each axis (x, y, z)

    # 1.1 Linearize the motion model and compute Jacobians
    F = np.identity(9);
    F[:3, 3:6] = delta_t * np.identity(3);
    F[3:6, 6:] = -skew_symmetric(rotationMatrix.dot(imu_f.data[k-1]))*delta_t;

    
    Q = np.identity(6);
    Q[:3, :3] = np.identity(3)*var_imu_f*delta_t**2;
    Q[3:, 3:] = np.identity(3)*var_imu_w*delta_t**2;
